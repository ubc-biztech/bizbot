...
class HealthCog(commands.Cog):
    ...
    @app_commands.command(
        name="db-connected", description="check db status"
    )
    async def get_item(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if 'TableNames' in page:
                ...
                message = f"✓ **[PASS] DB Status = Connected**\nList Tables:\n```json\n{tables}...\n```"
            else:
                message = f"✗ **[PASS] DB Status = Connected, unable to list tables**\n"

            await interaction.followup.send(message, ephemeral=True)

        except Exception as e:
            print(e)
            error_message = f"✗ **[FAIL] DB Status = Disconnected, Check AWS Credentials**\n"
